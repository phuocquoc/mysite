...
db = client.get_database("Cluster0")  # Lấy cơ sở dữ liệu mặc định, bạn có thể thay đổi nếu muốn
...
def get_cards(chat_id):
    return list(cards_collection.find({"chat_id": chat_id}, {"_id": 0, "id": 1, "name": 1, "due_day": 1, "last_paid_month": 1}))
...
def get_all_cards_for_reminder():
    ...
    return list(cards_collection.find({
        "due_day": {"$in": [day, day + 1]},
        "$or": [
            {"last_paid_month": None},
            {"last_paid_month": {"$ne": this_month}}
        ]
    }))
...
def update_card_paid(card_id):
    ...
    cards_collection.update_one({"_id": card_id}, {"$set": {"last_paid_month": current_month}})
